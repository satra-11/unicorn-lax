...
watch(() => props.photos, (_newPhotos, oldPhotos) => {
  if (!oldPhotos) return;
...
const togglePhoto = (photo: Photo) => {
  const updated = props.photos.map(p =>
    p.id === photo.id ? { ...p, excluded: !p.excluded } : p
  );
  emit('update:photos', updated);
};

const includedCount = computed(() =>
  props.photos.filter(p => !p.excluded).length
);

const matchedPhotos = computed(() =>
  props.photos.filter(p => !p.noFaceMatch)
);

const unmatchedPhotos = computed(() =>
  props.photos.filter(p => p.noFaceMatch)
);
</script>
      ...
      <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3">
        ...
        >
          <div class="aspect-video bg-gray-100 flex items-center justify-center overflow-hidden">
            ...
            />
            <span v-else class="text-gray-400 text-xs p-2 text-center truncate">{{ photo.name }}</span>
          </div>
