        ...
        <div class="p-6 space-y-8 flex-1 overflow-y-auto">
          ...
          <div>
              <div class="flex justify-between items-end mb-3">
                  <div>
                      <h3 class="text-sm font-semibold text-gray-900">学習データ (フィードバック)</h3>
              ...
              <div class="grid grid-cols-4 sm:grid-cols-5 gap-3 max-h-80 overflow-y-auto border border-gray-200 p-3 rounded-lg bg-gray-50/50">
                  <div 
                      ...
                      @click="toggleConfirm(photo.id)"
                  >
                      <img 
                          v-if="photo.thumbnail" 
                          :src="getPhotoUrl(photo)" 
                          class="w-full h-full object-cover transition-opacity duration-200"
                          :class="confirmedIds.has(photo.id) ? 'opacity-100' : 'opacity-60 group-hover:opacity-100 grayscale-[30%] group-hover:grayscale-0'"
                      />
                      ...
                      <!-- Checkmark Badge -->
                      <div 
                        v-if="confirmedIds.has(photo.id)" 
